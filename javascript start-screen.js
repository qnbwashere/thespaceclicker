    ...
    passwordInput.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        if (passwordInput.value === '8211') {
          ...
          // Close both password prompt and dev panel
          passwordPrompt.style.display = 'none';
...
// Append a style block to adjust the dev panel button position
document.addEventListener('DOMContentLoaded', () => {
  new SpaceBackground();
  new DevPanel();
  const devPanelButtonStyle = document.createElement('style');
  devPanelButtonStyle.textContent = `
    .dev-panel-button {
      position: absolute;
      bottom: -400px;
      left: 50%;
  ...
  `;
  document.head.appendChild(devPanelButtonStyle);
});